/**
 * Inline lucide SVG registry for zero-JS Astro chrome.
 *
 * The static header/footer/landing render no React, so `lucide-react` (a React
 * component) is unusable there. Each entry is the raw inner markup of the
 * matching lucide icon (24×24 viewBox, stroke-based) injected via `set:html` by
 * `icon.astro`. Paths transcribed verbatim from lucide so glyphs stay faithful.
 *
 * Keep this list tight — add an icon only when a chrome surface needs it.
 */
export const ICONS = {
  // Brand & identity
  scale:
  ...
  'list-ordered':
    '<path d="M10 12h11"/><path d="M10 6h11"/><path d="M10 18h11"/><path d="M4 6h1v4"/><path d="M4 10h2"/><path d="M6 18H4c0-1 2-2 2-3s-1-1.5-2-1"/>',
  'trending-up': '<path d="M16 7h6v6"/><path d="m22 7-8.5 8.5-5-5L2 17"/>',

  // Reassurance trio & misc affordances
  'shield-check':
